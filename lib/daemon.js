...
async function startDaemon(kbDir, { waitMs = 60000 } = {}) {
  ...
  // EDK2_KB_HOST: bind address (default 127.0.0.1). Set to 0.0.0.0 to expose
  // the MCP service to the LAN / a public tunnel.
  // EDK2_KB_PORT: fixed port (default 0 = OS-assigned dynamic port). A fixed
  // port keeps the MCP endpoint stable across restarts so opencode clients
  // only need to configure it once.
  const bindHost = process.env.EDK2_KB_HOST || '127.0.0.1';
  const bindPort = process.env.EDK2_KB_PORT;
  args.push('--host', bindHost);
  if (bindPort && /^\d+$/.test(bindPort)) {
    args.push('--port', bindPort);
  }
